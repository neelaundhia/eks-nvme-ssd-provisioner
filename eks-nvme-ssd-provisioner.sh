...
if [[ "$(ls -A /pv-disks)" ]]; then
  ...
  echo "Assuming that provisioning already happend, trying to assemble and mount!"
  case $SSD_NVME_DEVICE_COUNT in
  "0")
    exit 1
    ;;
  "1")
    echo "No need to assemble RAID for single device."
    DEVICE="${SSD_NVME_DEVICE_LIST[0]}"
    ;;
  *)
    # Checking if raid has already been started and is clean, if not try to assemble
...
echo "Device $DEVICE has been mounted to /pv-disks/$UUID"
echo "NVMe SSD provisioning is done, going to sleep."
